...
async function check() {
  ...
  assert.deepEqual(config.routes, [{ pattern: 'rutacentral.cl', custom_domain: true }]);
  assert.notEqual(config.workers_dev, false, 'Keep workers.dev available for migration redirects');
  ...
  const suffixes = ['/', '/assets/carta-ruta-central.pdf?download=1', '/?q=a%20b&x=%26&x=2', '/.git/HEAD', '/ruta%20central?next=%2Fmenu%3Fa%3D1'];
  for (const origin of ['http://rutacentral.cl', 'http://ruta-central.roberto-bravo-07.workers.dev',
    'https://ruta-central.roberto-bravo-07.workers.dev', 'https://www.rutacentral.cl',
    'http://ruta-central.example:8080', 'https://another.example']) {
    for (const suffix of suffixes) {
      const response = await worker.fetch(new Request(origin + suffix), env);
      assert.equal(response.status, 308, origin + suffix);
      checkHeaders(response);
      assert.equal(response.headers.get('location'), 'https://rutacentral.cl' + suffix);
    }
  }
  assert.equal(assetCalls, 0, 'Noncanonical requests must redirect before assets are read');
  for (const suffix of suffixes) {
    const redirected = await worker.fetch(new Request('https://ruta-central.roberto-bravo-07.workers.dev' + suffix), env);
    const canonicalRequest = new Request(redirected.headers.get('location'));
    const before = assetCalls;
    const response = await worker.fetch(canonicalRequest, env);
    assert.equal(assetCalls, before + 1);
    assert.equal(lastAssetRequest, canonicalRequest, 'Forward the original canonical request to ASSETS');
    assert.equal(response.headers.get('location'), null, 'Canonical redirect target must not loop');
    assert.equal(response.status, suffix.startsWith('/.git/') || suffix.startsWith('/ruta%20') ? 404 : 200);
    checkHeaders(response);
  }
  for (const relative of publicFiles) {
    const response = await worker.fetch(new Request('https://rutacentral.cl/' + relative), env);
  ...
  console.log('PASS: canonical rutacentral.cl and legacy/HTTP 308 redirects preserve path/query without loops; artifact bytes/references/exclusions, security headers on assets/redirects/404/304/206, body and cache metadata preserved (mock ASSETS).');
}
